...
function receiveData(req, res) {
  ...
  const { status, faultType, insight } = analyze(record);
  ...
  if (status !== 'active') {
    addAlert(record.panelId, faultType, insight, status === 'fault' ? 'critical' : 'warning');
  }

  console.log(`[DATA] ${record.timestamp} | Panel ${record.panelId} | Status: ${status} | ${insight}`);
  res.status(201).json({ received: true, status, insight });
}
